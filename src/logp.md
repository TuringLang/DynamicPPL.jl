...
In particular, the functions built here aim to evaluate the `logp`s for MCMC chains. 
...
function DynamicPPL.loglikelihood(model, data, chain)
    """
...
lls = DynamicPPL.loglikelihood(demo_model, x[1:10], chain) # a Dict with each key-value pair of size 3000x1
