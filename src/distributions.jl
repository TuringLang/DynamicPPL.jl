...
struct BinomialLogit{T<:Real,S<:Real} <: DiscreteUnivariateDistribution
    ...
    function BinomialLogit{T}(n::Int, logitp::T) where {T}
        ...
        logconstant = -(log1p(n) + n * LogExpFunctions.log1pexp(logitp))
...
function Base.rand(rng::Random.AbstractRNG, d::BinomialLogit)
    return rand(rng, Binomial(d.n, LogExpFunctions.logistic(d.logitp)))
end
function Distributions.sampler(d::BinomialLogit)
    return sampler(Binomial(d.n, LogExpFunctions.logistic(d.logitp)))
end
...
function unsafe_logpdf_ordered_logistic(η, cutpoints, K, k::Int)
    @inbounds begin
        logp = if k == 1
            -LogExpFunctions.log1pexp(η - cutpoints[k])
        elseif k < K
            tmp = LogExpFunctions.log1pexp(cutpoints[k - 1] - η)
            -tmp +
            LogExpFunctions.log1mexp(tmp - LogExpFunctions.log1pexp(cutpoints[k] - η))
        else
            -LogExpFunctions.log1pexp(cutpoints[k - 1] - η)
        end
    end
    return logp
end
