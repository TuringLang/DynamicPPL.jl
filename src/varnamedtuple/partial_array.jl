...
function empty_partialarray_from(data::AbstractArray)
    new_data = similar(data)
    new_mask = similar(data, Bool)
    fill!(new_mask, false)
...
"""
    _needs_arraylikeblock(value, inds::Vararg{INDEX_TYPES}; kw...)
...
"""
function _needs_arraylikeblock(value, inds::Vararg{INDEX_TYPES}; kw...)
    # TODO(penelopeysm) Not sure how to handle this. It could easily be a runtime check
    # but that might mess with constant propagation
    isempty(kw) ||
        throw(ArgumentError("Keyword indices are not supported for ArrayLikeBlocks"))
...
function BangBang.setindex!!(pa::PartialArray, value, inds::Vararg{INDEX_TYPES}; kw...)
    ...
    new_data = pa.data
    if _needs_arraylikeblock(value, inds...; kw...)
    ...
    fill!(view(pa.mask, inds...; kw...), true)
...
function _merge_recursive(pa1::PartialArray, pa2::PartialArray)
    ...
    for i in eachindex(pa1.mask)
        if pa1.mask[i]
            result = setindex!!(result, _merge_element_recursive(pa1, result, i), i)
        end
    end
    return result
end
