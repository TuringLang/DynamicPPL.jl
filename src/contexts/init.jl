...
Return the element type of the parameters generated by `strategy`.
...
Thread-safe evaluation uses this type to promote log-probability accumulators before
running the model. Known types permit concrete task-local storage; unknown or empty
parameter types permit storage to widen during evaluation, at a cost to type stability.
Non-threadsafe evaluation can promote accumulator types as values are accumulated.
