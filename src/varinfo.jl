...
function _invlink_metadata!(varinfo::VarInfo, metadata::Metadata)
    ...
    lengths_new = Int[]
    vals_new = mapreduce(vcat, vns) do vn
        x = getval(varinfo, vn)
        dist = getdist(varinfo, vn)
        # Return early if we're not transforming.
        !istrans(varinfo, vn) && vectorize(dist, x)
        # Transform to constrained space.
        f = invlink_transform(dist)
        y, logjac = with_logabsdet_jacobian_and_reconstruct(f, dist, x)
        # Vectorize value.
        yvec = vectorize(dist, y)
        # Add it to the new metadata.
        push!(lengths_new, length(yvec))
    ...
    ranges_new = similar(metadata.ranges, 0)
    offset = 0
    for l in lengths_new
        r_start, r_end = offset + 1, l + offset
        offset = r_end
        push!(ranges_new, r_start:r_end)
    end
