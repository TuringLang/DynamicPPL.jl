...
using SparseArrays
using StatsFuns: logaddexp, logistic, logit, log1pexp, logsumexp, softmax, loggamma, xlogy
using Statistics
using DynamicPPL
...
function Bijectors.VectorBijectors.scalar_to_scalar_bijector(::Flat)
    return Bijectors.VectorBijectors.TypedIdentity()
end
function Bijectors.VectorBijectors.scalar_to_scalar_bijector(d::FlatPos)
    return Bijectors.VectorBijectors.Log(d.l, 1)
end
...
Base.size(d::RBFKroneckerNormal) = (size(d.row_cholesky, 1), size(d.distances, 1))
Distributions.insupport(d::RBFKroneckerNormal, y::AbstractMatrix) = size(y) == size(d)
function Distributions.params(d::RBFKroneckerNormal)
    return (d.variance, d.bandwidth, d.row_cholesky, d.nugget, d.distances)
end

function Distributions._logpdf(d::RBFKroneckerNormal, y::AbstractMatrix{<:Real})
    column_cov = Symmetric(d.variance .* exp.(d.distances .* d.bandwidth) + 1.0e-5I)
    ...
    return -0.5 *
           (length(y) * log(2pi) + sum(abs2.(rotated) ./ variances) + sum(log, variances))
end
...
function _solve_ode(algorithm, f, y0, t0, ts, parameters, abstol, reltol, maxiters)
    save_times = ts.value
...
@model function arma_arma11(T, y)
    ...
    return y ~ product_distribution(map(Normal, nu, Fill(sigma, T)))
end
...
@model function earnings_log10earn_height(log10_earn, height)
    ...
    sigma ~ FlatPos(0.0)
    mu = beta[1] .+ beta[2] .* height
    return log10_earn ~ MvNormal(mu, sigma)
end

function make_model(::Val{Symbol("earnings-log10earn_height")}, data)
    return earnings_log10earn_height(
        log10.(Float64.(data["earn"])), Float64.(data["height"])
    )
end
...
@model function earnings_logearn_height(log_earn, height)
    ...
    sigma ~ FlatPos(0.0)
    mu = beta[1] .+ beta[2] .* height
    return log_earn ~ MvNormal(mu, sigma)
end
...
function make_model(::Val{Symbol("eight_schools-eight_schools_noncentered")}, data)
    return eight_schools_noncentered(
        data["J"], Float64.(data["y"]), Float64.(data["sigma"])
    )
end
...
@model function gp_pois_regr(dist_sq, k, N)
    ...
    f = factor.L * f_tilde
    DynamicPPL.@addlogprob! logpdf(product_distribution(map(x -> Poisson(exp(x)), f)), k)
end
...
@model function gp_regr(dist_sq, y, N)
    ...
    return y ~ MvNormal(zeros(N), Symmetric(covariance))
end

function make_model(::Val{Symbol("gp_pois_regr-gp_regr")}, data)
    x = Float64.(data["x"])
    return gp_regr(_FixedData((x .- x') .^ 2), _FixedData(Float64.(data["y"])), data["N"])
end
...
@model function kidiq_kidscore_momhs(kid_score, X)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma), size(kid_score))), kid_score .- X * beta
    )
end
...
function make_model(::Val{Symbol("kidiq_with_mom_work-kidscore_interaction_c")}, data)
    ...
    X = hcat(ones(length(mom_hs)), centered_hs, centered_iq, centered_hs .* centered_iq)
...
function make_model(::Val{Symbol("kidiq_with_mom_work-kidscore_interaction_c2")}, data)
    ...
    X = hcat(
        ones(length(centered_hs)), centered_hs, centered_iq, centered_hs .* centered_iq
    )
...
function make_model(::Val{Symbol("kidiq_with_mom_work-kidscore_mom_work")}, data)
    ...
    X = hcat(ones(length(mom_work)), mom_work .== 2, mom_work .== 3, mom_work .== 4)
...
@model function kilpisjarvi(x, y, pmualpha, psalpha, pmubeta, psbeta)
    ...
    sigma ~ FlatPos(0.0)
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma), size(y))), y .- (alpha .+ beta .* x)
    )
end
...
function make_model(::Val{Symbol("kilpisjarvi_mod-kilpisjarvi")}, data)
    return kilpisjarvi(
        Float64.(data["x"]),
        Float64.(data["y"]),
        Float64(data["pmualpha"]),
        Float64(data["psalpha"]),
        Float64(data["pmubeta"]),
        Float64(data["psbeta"]),
    )
end
...
function make_model(::Val{Symbol("mesquite-logmesquite")}, data)
    X = hcat(
        ones(length(data["weight"])),
        log.(data["diam1"]),
        log.(data["diam2"]),
        log.(data["canopy_height"]),
        log.(data["total_height"]),
        log.(data["density"]),
        data["group"],
    )
...
function make_model(::Val{Symbol("mesquite-logmesquite_logvas")}, data)
    ...
    log_area = log.(data["diam1"] .* data["diam2"])
    X = hcat(
        ones(length(log_volume)),
        log_volume,
        log_area,
        log.(data["diam1"] ./ data["diam2"]),
        log.(data["total_height"]),
        log.(data["density"]),
        data["group"],
    )
...
function make_model(::Val{Symbol("mesquite-logmesquite_logvolume")}, data)
    ...
    return mesquite_logmesquite_logvolume(log.(Float64.(data["weight"])), Float64.(X))
end
...
function make_model(::Val{Symbol("mesquite-mesquite")}, data)
    return mesquite_mesquite(
        ...
        Float64.(data["diam1"]),
        Float64.(data["diam2"]),
        Float64.(data["canopy_height"]),
        Float64.(data["total_height"]),
        Float64.(data["density"]),
        Float64.(data["group"]),
    )
end

@model function nes_model(
    partyid7, real_ideo, race_adj, educ1, gender, income, age_discrete
)
    ...
    mu =
        ...
        beta[5] .* age45_64 .+ beta[6] .* age65up .+ beta[7] .* educ1 .+
...
function _make_nes(data)
    return nes_model(
        Float64.(data["partyid7"]),
        Float64.(data["real_ideo"]),
        Float64.(data["race_adj"]),
        Float64.(data["educ1"]),
        Float64.(data["gender"]),
        Float64.(data["income"]),
...
@model function diamonds_model(stats, prior_only)
    ...
    if prior_only == 0
        sse =
            data.yty - 2dot(b, data.Xty) - 2Intercept * data.ysum +
            dot(b, data.XtX * b) +
            2Intercept * dot(data.xsum, b) +
            data.n * Intercept^2
...
function HiddenMarkovModel(n, initial, transition, emissions, transition_layout)
...
Distributions.insupport(d::HiddenMarkovModel, y::AbstractMatrix) = size(y) == size(d)
function Distributions.params(d::HiddenMarkovModel)
    return (d.initial, d.transition, d.emissions, d.transition_layout, d.first_observation)
end

function Distributions._logpdf(d::HiddenMarkovModel, observations::AbstractMatrix{<:Real})
    size(observations) == size(d) || return -Inf
    T, K = d.n, length(d.initial)
    emission = if d.emissions isa NormalEmissions
        means = if d.emissions.means isa AbstractVector
            reshape(d.emissions.means, 1, K)
        else
            d.emissions.means
        end
    ...
    alpha = alpha ./ scale
    transition =
        d.transition_layout isa Val{:rows} ? permutedims(d.transition) : d.transition
    for t in 2:T
...
@model function hmm_example_model(N, K, y, initial)
    ...
    mu ~ ordered(
        product_distribution([
            truncated(Normal(3, 1); lower=0), truncated(Normal(10, 1); lower=0)
        ]),
    )
    transition = [i == 1 ? theta1[j] : theta2[j] for i in 1:K, j in 1:K]
    emissions = NormalEmissions(mu, ones(K))
    return y ~ HiddenMarkovModel(N, initial, transition, emissions, Val(:rows))
end
...
@model function hmm_drive_0(K, observations, alpha, initial)
    ...
    phi ~ ordered(
        product_distribution([
            truncated(Normal(0, 1); lower=0), truncated(Normal(3, 1); lower=0)
        ]),
    )
    lambda ~ ordered(
        product_distribution([
    ...
    transition = [i == 1 ? theta1[j] : theta2[j] for i in 1:K, j in 1:K]
    emissions = [
        product_distribution([Exponential(inv(phi[k])), Exponential(inv(lambda[k]))]) for
        k in 1:K
    ]
    return observations ~ HiddenMarkovModel(
...
function make_model(::Val{Symbol("bball_drive_event_0-hmm_drive_0")}, data)
    ...
    return hmm_drive_0(data["K"], observations, Float64.(data["alpha"]), ones(data["K"]))
end
...
@model function hmm_drive_1(K, observations, alpha, tau, rho, initial)
    ...
    transition = [i == 1 ? theta1[j] : theta2[j] for i in 1:K, j in 1:K]
    emissions = [
        product_distribution([Normal(phi[k], tau), Normal(lambda[k], rho)]) for k in 1:K
    ]
    return observations ~ HiddenMarkovModel(
        size(observations, 1), initial, transition, emissions, Val(:rows)
    )
...
@model function lotka_volterra(N, t0, ts, y_init, fixed)
    ...
    z = _solve_ode(Tsit5(), lotka_volterra_rhs!, z_init, t0, ts, theta, 1e-3, 1e-5, 500)
    if minimum(z) <= 0
        DynamicPPL.@addlogprob! -Inf
        return nothing
    end
...
function make_model(::Val{Symbol("hudson_lynx_hare-lotka_volterra")}, data)
    fixed = _FixedData((
        log_y_transposed=permutedims(log.(Float64.(data["y"]))), zero_mean=zeros(2)
    ))
    return lotka_volterra(
        data["N"],
        _FixedData(0.0),
        _FixedData(Float64.(data["ts"])),
        Float64.(data["y_init"]),
        fixed,
    )
end
...
@model function one_comp_mm_elim_abs(t0, D, V, times, C_hat)
    ...
    one_compartment_rhs! = function (derivative, concentration, parameters, time)
        ...
        elimination =
            (maximum_rate / volume) * concentration[1] / (michaelis + concentration[1])
...
function make_model(::Val{Symbol("one_comp_mm_elim_abs-one_comp_mm_elim_abs")}, data)
    return one_comp_mm_elim_abs(
        _FixedData(Float64(data["t0"])),
        Float64(data["D"]),
        Float64(data["V"]),
        _FixedData(Float64.(data["times"])),
        Float64.(data["C_hat"]),
    )
end
...
@model function accel_gp(
    ...
    gp_constant = sdgp_1 * (sqrt(2pi) * lscale_1)^(slambda_dimension_1 / 2)
    gp_scale = gp_constant .* exp.(-0.25 * lscale_1^2 .* slambda_sq_1)
    mu = Intercept .+ Xgp_1 * (gp_scale .* zgp_1)
    sigma_constant =
        sdgp_sigma_1 * (sqrt(2pi) * lscale_sigma_1)^(slambda_sigma_dimension_1 / 2)
    sigma_scale = sigma_constant .* exp.(-0.25 * lscale_sigma_1^2 .* slambda_sigma_sq_1,)
...
function make_model(::Val{Symbol("mcycle_gp-accel_gp")}, data)
    ...
    return accel_gp(
        Float64.(data["Y"]),
        Float64.(data["Xgp_1"]),
        vec(sum(abs2, slambda_1; dims=2)),
        size(slambda_1, 2),
        Float64.(data["Xgp_sigma_1"]),
        vec(sum(abs2, slambda_sigma_1; dims=2)),
        size(slambda_sigma_1, 2),
        data["NBgp_1"],
        data["NBgp_sigma_1"],
        data["prior_only"],
    )
...
@model function pdb_GLMM_Poisson_model(n, C, year, year_squared, year_cubed)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma), size(eps))), eps
    )
end

function make_model(::Val{Symbol("GLMM_Poisson_data-GLMM_Poisson_model")}, data)
    year = data["year"]
    return pdb_GLMM_Poisson_model(data["n"], data["C"], year, year .^ 2, year .^ 3)
end
...
function make_model(::Val{Symbol("GLM_Binomial_data-GLM_Binomial_model")}, data)
    year = data["year"]
    return pdb_GLM_Binomial_model(data["nyears"], data["C"], data["N"], year, year .^ 2)
end
...
function make_model(::Val{Symbol("GLM_Poisson_Data-GLM_Poisson_model")}, data)
    year = data["year"]
    return pdb_GLM_Poisson_model(data["n"], data["C"], year, year .^ 2, year .^ 3)
end
...
@model function pdb_Mh_model(M, T, y)
    ...
    for i in 1:M
        if y[i] > 0
            DynamicPPL.@addlogprob! logpdf(Bernoulli(omega), 1) +
                logpdf(BinomialLogit(T, eps[i]), y[i])
        else
            DynamicPPL.@addlogprob! logaddexp(
                logpdf(Bernoulli(omega), 1) + logpdf(BinomialLogit(T, eps[i]), 0),
                logpdf(Bernoulli(omega), 0),
            )
        end
    end
...
@model function pdb_Mtbh_model(M, T, y, s)
    ...
    for i in 1:M
        if s[i] > 0
            ...
            DynamicPPL.@addlogprob! logaddexp(
                logpdf(Bernoulli(omega), 1) +
                logpdf(product_distribution(map(BernoulliLogit, logit_p[i, :])), y[i, :]),
                logpdf(Bernoulli(omega), 0),
            )
        end
    end
...
@model function pdb_Survey_model(successes, failures, log_coefficients)
    theta ~ Uniform(0, 1)
    terms = log_coefficients .+ successes * log(theta) .+ failures .* log1p(-theta)
...
function make_model(::Val{Symbol("Survey_data-Survey_model")}, data)
    ...
    log_coefficients = [
        -log(nmax) + sum(ki -> logpdf(Binomial(n, 0.5), ki), k) - m * n * log(0.5) for
        n in ns
    ]
...
@model function pdb_accel_splines(
    N,
    Y,
    Ks,
    Xs,
...
function make_model(::Val{Symbol("mcycle_splines-accel_splines")}, data)
    return pdb_accel_splines(
        data["N"],
        _FixedData(Float64.(data["Y"])),
        data["Ks"],
        _FixedData(Float64.(data["Xs"])),
        data["knots_1"],
        _FixedData(Float64.(data["Zs_1_1"])),
        data["Ks_sigma"],
        _FixedData(Float64.(data["Xs_sigma"])),
        data["knots_sigma_1"],
        _FixedData(Float64.(data["Zs_sigma_1_1"])),
        data["prior_only"],
    )
end
...
@model function pdb_covid19imperial_v2(M, P, fixed, observed_deaths)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(3.28, kappa), size(mu))), mu
    )
    ...
    for m in 1:M
        ...
        for i in (data.N0 + 1):(data.N2)
            ...
            reproduction = mu[m] * exp(-dot(view(data.X, m, i, :), adjusted_alpha))
    ...
    DynamicPPL.@addlogprob! sum(loggamma.(observed_deaths .+ phi)) -
                            length(observed_deaths) * loggamma(phi) -
                            data.count_logfactorial +
...
function _make_covid(data)
    ...
    observed_deaths = [
        deaths[i, m] for m in 1:data["M"] for i in epidemic_start[m]:observation_end[m]
    ]
    fixed = _FixedData((
        N0=Int(data["N0"]),
        N2=N2,
    ...
    return pdb_covid19imperial_v2(data["M"], data["P"], fixed, observed_deaths)
end
...
@model function pdb_dogs_hierarchical(n_dogs, n_trials, y, prev_shock, prev_avoid)
    J = n_dogs
    T = n_trials
    a ~ Uniform(0, 1)
    b ~ Uniform(0, 1)
...
function make_model(::Val{Symbol("dogs-dogs_hierarchical")}, data)
    ...
    prev_shock = hcat(zeros(n_dogs), cumsum(past; dims=2))
    prev_avoid = hcat(zeros(n_dogs), cumsum(1 .- past; dims=2))
    return pdb_dogs_hierarchical(n_dogs, n_trials, y, prev_shock, prev_avoid)
end
...
@model function pdb_dogs_log(n_dogs, n_trials, y, prev_shock, prev_avoid)
    ...
    for i in 1:n_dogs
        for j in 1:n_trials
            p = logistic(beta[1] * prev_avoid[i, j] + beta[2] * prev_shock[i, j])
...
function make_model(::Val{Symbol("dogs-dogs_nonhierarchical")}, data)
    ...
    prev_shock = hcat(zeros(n_dogs), cumsum(past; dims=2))
    prev_avoid = hcat(zeros(n_dogs), cumsum(1 .- past; dims=2))
    return pdb_dogs_nonhierarchical(n_dogs, n_trials, y, prev_shock, prev_avoid)
end
...
@model function pdb_election88_full(
    N,
    n_age,
    n_age_edu,
    n_edu,
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma_b), size(b))), b
    )
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma_c), size(c))), c
    )
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma_d), size(d))), d
    )
    DynamicPPL.@addlogprob! logpdf(
...
@model function pdb_gpcm_latent_reg_irt(
    I, J, N, ii, jj, response, m, W_adj, levels, valid, response_indices, step_design
)
    K = size(W_adj, 2)
    ...
    cumulative_difficulty = reshape(step_design * beta, I, size(levels, 2))
    logits = reshape(ability, N, 1) .* levels .- cumulative_difficulty[ii, :]
    logits = ifelse.(valid, logits, -Inf)
    DynamicPPL.@addlogprob! sum(logits[response_indices]) - sum(logsumexp(logits; dims=2))
end
...
function make_model(::Val{Symbol("timssAusTwn_irt-gpcm_latent_reg_irt")}, data)
    ...
    response = y .+ 1
    response_indices = [n + (response[n] - 1) * data["N"] for n in 1:data["N"]]
...
@model function pdb_grsm_latent_reg_irt(
    I, J, N, ii, jj, response, max_category, W_adj, levels, response_indices
)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, 3), size(kappa))), kappa
    )
    ability = alpha[ii] .* theta[jj] .- beta[ii]
    cumulative_kappa = vcat(zero(kappa[1]), cumsum(kappa))
    logits = reshape(ability, N, 1) .* levels .- reshape(cumulative_kappa, 1, :)
    DynamicPPL.@addlogprob! sum(logits[response_indices]) - sum(logsumexp(logits; dims=2))
end
...
function make_model(::Val{Symbol("sat-hier_2pl")}, data)
    ...
    return pdb_hier_2pl(data["I"], data["J"], data["N"], data["ii"], data["jj"], signs)
end

# PosteriorDB Stan model: hierarchical_gp
@model function pdb_hierarchical_gp(
    N,
    N_states,
    N_regions,
    N_years_obs,
...
function make_model(::Val{Symbol("state_wide_presidential_votes-hierarchical_gp")}, data)
    return pdb_hierarchical_gp(
        data["N"],
        data["N_states"],
        data["N_regions"],
        data["N_years_obs"],
        data["N_years"],
        data["state_region_ind"],
        data["state_ind"],
        data["region_ind"],
        data["year_ind"],
        data["y"],
...
@model function pdb_hmm_gaussian(T, K, y)
    ...
    return y ~ HiddenMarkovModel(T, pi1, A, emissions, Val(:columns), Val(:summed_first))
end
...
@model function pdb_iohmm_reg(T, K, M, y, u)
    ...
    return y ~ HiddenMarkovModel(T, pi1, logA, emissions, Val(:source_logprob))
end

function make_model(::Val{Symbol("iohmm_reg_simulated-iohmm_reg")}, data)
    y = reshape(Float64.(data["y"]), :, 1)
    return pdb_iohmm_reg(data["T"], data["K"], data["M"], y, Float64.(data["u"]))
end
...
@model function pdb_irt_2pl(I, J, y)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma_theta), size(theta))), theta
    )
...
@model function pdb_kronecker_gp(n1, n2, xd, y)
    ...
    Lm = Matrix(L.L)
    return y ~ RBFKroneckerNormal(var1, bw1, Lm, sigma1, xd)
end

function make_model(::Val{Symbol("synthetic_grid_RBF_kernels-kronecker_gp")}, data)
    x = Float64.(data["x1"])
    xd = -((x .- x') .^ 2)
    return pdb_kronecker_gp(data["n1"], data["n2"], xd, Matrix{Float64}(data["y"]))
end
...
@model function pdb_ldaK2(V, M, N, w, doc)
    ...
    logits = permutedims(log.(view(theta, :, doc))) .+ log.(view(phi, w, :))
...
function make_model(::Val{Symbol("prideprejudice_chapter-ldaK5")}, data)
    return pdb_ldaK5(
        data["V"], data["M"], data["N"], data["w"], data["doc"], data["alpha"], data["beta"]
    )
end
...
@model function pdb_logistic_regression_rhs(
    n, d, y, x, scale_icept, scale_global, nu_global, nu_local, slab_scale, slab_df
)
    x = x.value
    beta0 ~ Normal(0.0, scale_icept)
    z ~ product_distribution(Fill(Normal(), d))
    tau ~ truncated(LocationScale(0.0, 2.0scale_global, TDist(nu_global)); lower=0.0)
    lambda ~ product_distribution(
        Fill(truncated(LocationScale(0.0, 1.0, TDist(nu_local)); lower=0.0), d)
    )
...
function make_model(::Val{Symbol("ovarian-logistic_regression_rhs")}, data)
    ...
    return pdb_logistic_regression_rhs(
        data["n"],
        data["d"],
        data["y"],
        X,
...
@model function pdb_losscurve_sislob(
    growthmodel_id,
    n_data,
    n_time,
    n_cohort,
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(), size(loss))), (loss .- location) ./ scale
    ) - sum(log, scale)
end

function make_model(::Val{Symbol("loss_curves-losscurve_sislob")}, data)
    return pdb_losscurve_sislob(
        data["growthmodel_id"],
        data["n_data"],
        data["n_time"],
        data["n_cohort"],
...
function make_model(
    ::Val{Symbol("low_dim_gauss_mix_collapse-low_dim_gauss_mix_collapse")}, data
)
...
@model function pdb_multi_occupancy(J, K, n, X, S)
    ...
    for i in (n + 1):S
        ...
        DynamicPPL.@addlogprob! logaddexp(
            logpdf(Bernoulli(Omega), 0), logpdf(Bernoulli(Omega), 1) + J * unobserved
        )
    end
end
...
@model function pdb_nes_logit_model(X, vote)
    ...
    logits = alpha .+ X * beta
    return vote ~ product_distribution(map(BernoulliLogit, logits))
end
...
@model function pdb_nn_rbm1bJ10(N, M, x, K, y)
    ...
    return y ~ product_distribution(
        map(axes(logits, 1)) do i
...
@model function pdb_prophet(
    ...
    if trend_indicator == 0
        ...
        DynamicPPL.@addlogprob! logpdf(
            product_distribution(Fill(Normal(0, sigma_obs), T)), y .- prediction
        )
    elseif trend_indicator == 1
        ...
        for i in 1:S
            gamma[i] =
                (t_change[i] - shifted_offset) * (1 - segment_rate[i] / segment_rate[i + 1])
            shifted_offset += gamma[i]
        end
        trend = cap .* logistic.((k .+ A * delta) .* ((t .- m) .- A * gamma),)
        prediction = trend .* (1 .+ X_m * beta) + X_a * beta
        DynamicPPL.@addlogprob! logpdf(
            product_distribution(Fill(Normal(0, sigma_obs), T)), y .- prediction
        )
...
@model function pdb_radon_county(J, county_design, y)
    ...
    return y ~ MvNormal(prediction, sigma_y)
end

function make_model(::Val{Symbol("radon_mod-radon_county")}, data)
    N = data["N"]
    J = data["J"]
    county_design = sparse(1:N, Int.(data["county"]), ones(Float64, N), N, J)
    return pdb_radon_county(J, _FixedData(county_design), Float64.(data["y"]))
end
...
function make_model(::Val{Symbol("radon_all-radon_county_intercept")}, data)
    return pdb_radon_county_intercept(
        data["N"], data["J"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
@model function pdb_radon_hierarchical_intercept_centered(
...
function make_model(::Val{Symbol("radon_all-radon_hierarchical_intercept_centered")}, data)
    return pdb_radon_hierarchical_intercept_centered(
        data["J"],
        data["N"],
        data["county_idx"],
        data["log_uppm"],
...
function make_model(
    ::Val{Symbol("radon_all-radon_hierarchical_intercept_noncentered")}, data
)
    return pdb_radon_hierarchical_intercept_noncentered(
        data["J"],
        data["N"],
        data["county_idx"],
        data["log_uppm"],
...
function make_model(
    ::Val{Symbol("radon_mn-radon_hierarchical_intercept_noncentered")}, data
)
    return pdb_radon_hierarchical_intercept_noncentered(
        data["J"],
        data["N"],
...
function make_model(::Val{Symbol("radon_all-radon_partially_pooled_centered")}, data)
    return pdb_radon_partially_pooled_centered(
        data["N"], data["J"], data["county_idx"], data["log_radon"]
    )
end
...
function make_model(::Val{Symbol("radon_all-radon_partially_pooled_noncentered")}, data)
    return pdb_radon_partially_pooled_noncentered(
        data["N"], data["J"], data["county_idx"], data["log_radon"]
    )
end
...
@model function pdb_radon_variable_intercept_centered(
...
function make_model(::Val{Symbol("radon_all-radon_variable_intercept_centered")}, data)
    return pdb_radon_variable_intercept_centered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
function make_model(::Val{Symbol("radon_all-radon_variable_intercept_noncentered")}, data)
    return pdb_radon_variable_intercept_noncentered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
@model function pdb_radon_variable_intercept_slope_centered(
    J, N, county_idx, floor_measure, log_radon
)
    sigma_y ~ FlatPos(0)
    sigma_alpha ~ FlatPos(0)
...
function make_model(
    ::Val{Symbol("radon_all-radon_variable_intercept_slope_centered")}, data
)
    return pdb_radon_variable_intercept_slope_centered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
@model function pdb_radon_variable_intercept_slope_noncentered(
    J, N, county_idx, floor_measure, log_radon
)
    sigma_y ~ FlatPos(0)
    sigma_alpha ~ FlatPos(0)
...
function make_model(
    ::Val{Symbol("radon_all-radon_variable_intercept_slope_noncentered")}, data
)
    return pdb_radon_variable_intercept_slope_noncentered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end

function make_model(
    ::Val{Symbol("radon_mn-radon_variable_intercept_slope_noncentered")}, data
)
    return pdb_radon_variable_intercept_slope_noncentered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
...
function make_model(::Val{Symbol("radon_all-radon_variable_slope_centered")}, data)
    return pdb_radon_variable_slope_centered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
function make_model(::Val{Symbol("radon_all-radon_variable_slope_noncentered")}, data)
    return pdb_radon_variable_slope_noncentered(
        data["J"], data["N"], data["county_idx"], data["floor_measure"], data["log_radon"]
    )
end
...
@model function pdb_rats_model(N, Npts, rat, x, y, xbar)
    ...
    for n in 1:Npts
        irat = rat[n]
        DynamicPPL.@addlogprob! logpdf(
            Normal(alpha[irat] + beta[irat] * (x_[n] - xbar), sigma_y), y[n]
        )
    end
end

function make_model(::Val{Symbol("rats_data-rats_model")}, data)
    return pdb_rats_model(
        data["N"], data["Npts"], data["rat"], data["x"], data["y"], data["xbar"]
    )
end
...
@model function pdb_seeds_centered_model(I, n, N, x1, x2, x1x2)
    ...
    DynamicPPL.@addlogprob! logpdf(product_distribution(map(BinomialLogit, N, logits)), n)
end

function make_model(::Val{Symbol("seeds_data-seeds_centered_model")}, data)
    x1, x2 = data["x1"], data["x2"]
    return pdb_seeds_centered_model(data["I"], data["n"], data["N"], x1, x2, x1 .* x2)
end
...
function make_model(::Val{Symbol("seeds_data-seeds_model")}, data)
    x1, x2 = data["x1"], data["x2"]
    return pdb_seeds_model(data["I"], data["n"], data["N"], x1, x2, x1 .* x2)
end
...
function make_model(::Val{Symbol("seeds_data-seeds_stanified_model")}, data)
    x1, x2 = data["x1"], data["x2"]
    return pdb_seeds_stanified_model(data["I"], data["n"], data["N"], x1, x2, x1 .* x2)
end
...
@model function pdb_sir(N_t, t0, t, y0, stoi_hat, B_hat)
    ...
    sir_rhs! = function (derivative, state, theta, _)
        ...
        derivative[2] =
            theta[1] * state[4] / (state[4] + theta[2]) * state[1] - theta[3] * state[2]
        derivative[3] = theta[3] * state[2]
        derivative[4] = theta[4] * state[2] - theta[5] * state[4]
        return nothing
    end
    y = _solve_ode(
        DP5(), sir_rhs!, y0, t0, t, [beta, kappa, gamma, xi, delta], 1e-6, 1e-6, 100_000_000
    )
    DynamicPPL.@addlogprob! logpdf(Cauchy(0, 2.5), beta)
    ...
    for n in 2:N_t
        DynamicPPL.@addlogprob! logpdf(
            Poisson(max(1.0e-16, y[n - 1, 1] - y[n, 1])), stoi_hat[n]
        )
    end
    if any(y[:, 4] .<= 0)
        DynamicPPL.@addlogprob! -Inf
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(map(m -> LogNormal(m, 0.15), log.(y[:, 4]))), B_hat
    )
end
...
@model function pdb_soil_incubation(totalC_t0, t0, N_t, ts, eCO2mean)
    ...
    eCO2_hat = map(ts.value) do time
        ...
        carbon1 = common * (cosh(gap_time) * carbon1_t0 + hyperbolic * transformed1)
    ...
    DynamicPPL.@addlogprob! logpdf(Cauchy(0, 1), sigma)
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma), size(eCO2mean))), eCO2mean .- eCO2_hat
    )
end
...
@model function pdb_state_space_stochastic_level_stochastic_seasonal(
    n, y, x, w, seasonal_design, mu_lower, mu_upper
)
    ...
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma[2]), n - 1)), mu[2:n] .- mu[1:(n - 1)]
    )
    prediction = mu .+ beta .* x .+ lambda .* w .+ seasonal
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(Normal(0, sigma[3]), n)), y .- prediction
    )
    DynamicPPL.@addlogprob! logpdf(
        product_distribution(Fill(LocationScale(0, 1, TDist(4)), size(sigma))), sigma
    )
end

function make_model(
    ::Val{Symbol("uk_drivers-state_space_stochastic_level_stochastic_seasonal")}, data
)
    n = data["n"]
    y = Float64.(data["y"])
    ...
    return pdb_state_space_stochastic_level_stochastic_seasonal(
        n, y, Float64.(data["x"]), Float64.(data["w"]), seasonal_design, mu_lower, mu_upper
    )
end
...
@model function pdb_surgical_model(N, r, n)
    ...
    DynamicPPL.@addlogprob! logpdf(product_distribution(map(BinomialLogit, n, b)), r)
end
...
function make_model(::Val{Symbol("wells_data-wells_daae_c_model")}, data)
    ...
    X = hcat(c_dist100, c_arsenic, c_dist100 .* c_arsenic, data["assoc"], data["educ"] ./ 4)
...
function Mooncake.rrule!!(
    ...
    value =
        -0.5 *
        (length(py) * log(2pi) + sum(abs2.(rotated) ./ variances) + sum(log, variances))
    ...
    row_gradient =
        0.5 .*
        (alpha * column_cov * alpha' - row_vectors * Diagonal(row_trace) * row_vectors')
    variance_gradient = dot(column_gradient, exponential)
    bandwidth_gradient = dot(column_gradient, d.variance .* exponential .* d.distances)
    ...
    for j in axes(column_storage_gradient, 2)
        ...
        for i in firstindex(column_storage_gradient, 1):(j - 1)
            column_storage_gradient[i, j] = column_gradient[i, j] + column_gradient[j, i]
        end
    end
    distances_gradient =
        column_storage_gradient .* (d.variance * d.bandwidth .* exponential)
