...
Tables.getcolumn(row::TrialJudgementRow, i::Int) = Tables.getcolumn(row, Tables.columnnames(row)[i])
...
function Tables.columns(rows::Vector{<:TrialJudgementRow})
    return (; ((name, getproperty.(rows, name)) for name in Tables.columnnames(eltype(rows)))...)
end
...
function Base.show(io::IO, ::MIME"text/html", rows::Vector{<:TrialJudgementRow})
    hgood = HTMLHighlighter(make_highlighter_judgement(true), HTMLDecoration(color="green", font_weight="bold"))
    hbad = HTMLHighlighter(make_highlighter_judgement(false), HTMLDecoration(color="red", font_weight="bold"))
    formatter = make_formatter(rows)
    pretty_table(
        io, rows,
        backend=Val(:html),
...
function judgementtable(
    results::AbstractVector, results_old::AbstractVector,
...
function judgementtable(name::String, name_old::String; kwargs...)
    ...
    for model_name in model_names
        ...
        append!(results_old, BenchmarkTools.load(projectdir("results", name_old, "$(model_name).json")))
    end
