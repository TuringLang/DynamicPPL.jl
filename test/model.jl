...
@testset "model.jl" begin
    @testset "convenience functions" begin
        ...
        for model in TU.DEMO_MODELS # length(TU.DEMO_MODELS)=12
            ...
            for i in 1:N
                ...
                @test logpriors[i] ≈
                    TU.logprior_true(model, samples[:s], samples[:m])
                @test loglikelihoods[i] ≈ TU.loglikelihood_true(
                    model, samples[:s], samples[:m]
                )
                @test logjoints[i] ≈
                    TU.logjoint_true(model, samples[:s], samples[:m])
            end
        end
    end
    ...
    if VERSION >= v"1.8"
        @testset "Type stability of models" begin
            models_to_test = [
                TU.DEMO_MODELS..., TU.demo_lkjchol(2)
            ]
            ...
            @testset "$(model.f)" for model in models_to_test
                ...
                varinfos = filter(
                    is_typed_varinfo,
                    TU.setup_varinfos(model, example_values, vns),
                )
