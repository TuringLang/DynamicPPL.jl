...
@testset "varinfo.jl" begin
    ...
    @testset "unflatten + linking" begin
        @testset "Model: $(model.f)" for model in [
            TU.demo_one_variable_multiple_constraints(), TU.demo_lkjchol()
        ]
    ...
    @testset "subset" begin
        ...
        varinfos = TU.setup_varinfos(model, model(), vns; include_threadsafe=true)
    ...
    @testset "merge" begin
        @testset "$(model.f)" for model in TU.DEMO_MODELS
            vns = TU.varnames(model)
            varinfos = TU.setup_varinfos(
                model, TU.rand_prior_true(model), vns; include_threadsafe=true
            )
            @testset "$(TU.short_varinfo_name(varinfo))" for varinfo in varinfos
                ...
                @testset "with different value" begin
                    ...
                    varinfo_changed = TU.update_values!!(deepcopy(varinfo), x, vns)
